...
This file contains details on each script that is available in the `slr-kit` repository.
Each script can be run as a standalone program.
Althouh, typically, they are run by the main `slrkit` command.
...
usage: merge_labels.py [-h] old new FILENAME
